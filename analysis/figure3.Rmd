...
ggsave("/projects/mludwig/Petersen-2022/output/figures/figure3e.png", volcano.plot, height = 60, width = 90,
       units = "mm")
```
...
write.xlsx(DEGs.conjugate.save, file = "/projects/mludwig/Petersen-2022/output/DEGs/DEGs_conjugate_vs_vehicle.xlsx",
           rowNames = F)
```
...
ggsave("/projects/mludwig/Petersen-2022/output/figures/figure3g.png", GO.plot, height = 42, width = 70,
       units = "mm")
```
...
ggsave("/projects/mludwig/Petersen-2022/output/figures/figure3f.png", GO.genes.plot, height = 60, width = 180,
       units = "mm")
```       

<!-- # GWAS plot -->
<!-- ```{r} -->
<!-- LDSC.output <- read.csv("/projects/mludwig/Petersen-2022/output/LDSC/LDSC_output_GO_genes_conjugate_vs_vehicle.txt", -->
<!--                         sep = "\t") -->
<!-- LDSC.output$Name <- gsub("GO_genes_conjugate_vs_vehicle__", "", LDSC.output$Name) -->
<!-- LDSC.output <- LDSC.output[, c("Name", "Coefficient_P_value")] -->
<!-- colnames(LDSC.output) <- c("term", "p.value") -->
<!-- LDSC.output$method <- "LDSC" -->

<!-- MAGMA.output <- read.csv("/projects/mludwig/Petersen-2022/output/MAGMA/MAGMA_output_GO_genes_conjugate_vs_vehicle.gsa.out", -->
<!--                          skip = 3, sep = "") -->
<!-- MAGMA.output <- MAGMA.output[, c("VARIABLE", "P")] -->
<!-- colnames(MAGMA.output) <- c("term", "p.value") -->
<!-- MAGMA.output$method <- "MAGMA" -->


<!-- GWAS.data <- rbind(LDSC.output, MAGMA.output) -->
<!-- GWAS.data$p.adj <- p.adjust(GWAS.data$p.value, method = "fdr") -->
<!-- GWAS.data$term <- gsub("_", " ", GWAS.data$term) -->
<!-- GWAS.data$term <- factor(GWAS.data$term, levels = rev(GO.terms$term_name)) -->

<!-- for (i in 1:nrow(GWAS.data)) { -->
<!--   term <- as.character(GWAS.data$term[i]) -->
<!--   n.genes <- length(GO.list[[term]]$gene) -->
<!--   GWAS.data$n.genes[i] <- n.genes -->
<!-- } -->

<!-- GWAS.data$term <- paste0(GWAS.data$term, "\n (", GWAS.data$n.genes, " genes)") -->
<!-- GWAS.data$term <- factor(GWAS.data$term, levels = rev(c("neuron to neuron synapse\n (125 genes)", -->
<!--                                                         "glutamatergic synapse\n (138 genes)", -->
<!--                                                         "asymmetric synapse\n (115 genes)", -->
<!--                                                         "postsynaptic specialization\n (120 genes)", -->
<!--                                                         "postsynaptic density\n (112 genes)"))) -->
<!-- GWAS.data$method <- factor(GWAS.data$method, levels = c("LDSC", "MAGMA")) -->
<!-- ``` -->

<!-- ```{r} -->
<!-- GWAS.plot <- ggplot(GWAS.data, aes(term, -log10(p.adj), fill = method)) +  -->
<!--   geom_col(colour="black", alpha=1, size=0.3, width = 0.8, position = "dodge") + -->
...
<!--         plot.margin = unit(c(0, 0.5, 0, 0), "cm")) + -->
<!--   coord_flip() + -->
<!--   xlab(NULL) + ylab(expression(paste(bold(-log[10]),bold("("), bolditalic("P"), -->
<!--                                    bold(")")))) +  -->
<!--     geom_hline(yintercept = -log10(0.05), lty = 2) + -->
<!--   scale_fill_manual(values = c("darkseagreen", "darkslategray"), limits = c("MAGMA", "LDSC")) -->
...
genes.conjugate <- rownames(DEGs.conjugate)[which(DEGs.conjugate$padj < 0.05)]

genes.union <- union(genes.peptide, genes.conjugate)
genes.intersect <- intersect(genes.peptide, genes.conjugate)

genes.common <- data.frame(cbind(DEGs.peptide[genes.union, "log2FoldChange"], 
                                 DEGs.conjugate[genes.union, "log2FoldChange"]))  
...
# Plot
plot.3x <- ggplot(genes.common, aes(x = Peptide, y = Conjugate, fill = treatment)) + 
  geom_point(shape = 21, size = 3, alpha = 1)  +
  xlim(-5,5) + ylim(-5,5) +
  xlab(expression(bold(atop("Log"[2] *" fold-change", "GLP-1")))) +
  ...
  scale_fill_manual(values = pal, labels = c("GLP-1", "GLP-1/MK-801", "GLP-1 &\nGLP-1/MK-801")) +
...
write.xlsx(genes.common,
           file = "/projects/mludwig/Petersen-2022/output/DEGs/genes_cor_conjugate_vs_peptide.xlsx",
           rowNames = F)
```

# Figure 3y
```{r}
genes.molecule <- rownames(DEGs.molecule)[which(DEGs.molecule$padj < 0.05)]
genes.conjugate <- rownames(DEGs.conjugate)[which(DEGs.conjugate$padj < 0.05)]

genes.union <- union(genes.molecule, genes.conjugate)
genes.intersect <- intersect(genes.molecule, genes.conjugate)

genes.common <- data.frame(cbind(DEGs.molecule[genes.union, "log2FoldChange"], 
                                 DEGs.conjugate[genes.union, "log2FoldChange"]))  

rownames(genes.common) <- genes.union

colnames(genes.common) <- c("Molecule", "Conjugate")
genes.common$treatment <- ""
genes.common$treatment[which(genes.union %in% genes.molecule)] <- "Molecule"
genes.common$treatment[which(genes.union %in% genes.conjugate)] <- "Conjugate"
genes.common$treatment[which(genes.union %in% genes.intersect)] <- "Molecule &\nconjugate"
genes.common$treatment <- factor(genes.common$treatment, 
                                levels = c("Molecule", "Conjugate", "Molecule &\nconjugate"))
...
# Plot
plot.3y <- ggplot(genes.common, aes(x = Molecule, y = Conjugate, fill = treatment)) + 
  geom_point(shape = 21, size = 3, alpha = 1)  +
  xlim(-5,5) + ylim(-5,5) +
  xlab(expression(bold(atop("Log"[2] *" fold-change", "MK-801")))) +
  ylab(expression(bold(atop("Log"[2] *" fold-change", "GLP-1/MK-801")))) +
  theme_pubr(legend="top") + theme(panel.border = element_blank(), 
                     panel.grid.major = element_blank(),
                     panel.grid.minor = element_blank(), 
                     ...
                     legend.spacing.x = unit(-0.005, "cm"),
                     plot.margin = unit(c(0, 0, 0, 0), "cm")) +
  scale_fill_manual(values = pal, labels = c("MK-801", "GLP-1/MK-801", "MK-801 &\nGLP-1/MK-801")) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed")
...
genes.common <- cbind(gene = rownames(genes.common), genes.common)
write.xlsx(genes.common,
           file = "/projects/mludwig/Petersen-2022/output/DEGs/genes_cor_conjugate_vs_molecule.xlsx",
           rowNames = F)
```
